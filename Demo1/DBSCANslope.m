...
%             segment_pos2 = segment_pos - X(i,1);
%             position = find(segment_pos2>0,1,'first');
%             if isempty(position) == 1
%                 slope_ref = slope_seg(end);
%             else
%                 slope_ref = slope_seg(position); 
%             end
%             if slope_ref>= 0
%                 ubac_slope = 0*ubac_slope + 1*slope_ref;
%             else
%                 adret_slope = 0*adret_slope + 1*slope_ref;
%             end
            %-------------------------------------
            ...
            if (numel(Neighbors_adret)>=MinPts) || (numel(Neighbors_ubac)>=MinPts)
                if numel(Neighbors_adret) >= numel(Neighbors_ubac)
                    FindNeighbour(i,Neighbors_adret);
                else
                    FindNeighbour(i,Neighbors_ubac);
                end
            end
        end
    end

    function FindNeighbour(i,Neighbors)
        FLAG = FLAG + 1;
        IDX(i,1)=FLAG;
        k = 1;
        while true
