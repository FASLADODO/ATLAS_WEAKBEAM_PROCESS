...
load('data_20190325013757.mat');%load geolocation photon data of strong beam
...
load('data_20190325013757_dbscan.mat');%load DBSCAN filtered result of strong beam
...
tic
parfor kk=1:al_num
    [classify_all,noise_all,classify,dist_r,elev_r,lat_r,lon_r,slope_r,noise_r,minpts_r,snr_r]=filter_DBSCANslope(d{kk},h{kk},b{kk},l{kk},fitresult_ns_adret,fitresult_ns_ubac,segment_pos,slope_seg);
    TBLHC_dbscannew=[];
...
for i = 1:al_num
    ...
    PE_noise = [PE_noise;noise_rec{i}];
end
PE_type(1) = [];
PE_noise(1) = [];
TBLHC2 = [TBLHC2,PE_noise,PE_type];
temp2(1,:) = [];
...
index_final3 = find(temp_filter_input2(:,end)==-1);
index_final4 = find(temp_filter_input2(:,end)==-2);
scatter(TBLHC2(:,1),TBLHC2(:,2),'.');
hold on
scatter(temp_filter_input2(index_final2,1),temp_filter_input2(index_final2,2),'.');
scatter(temp_filter_input2(index_final3,1),temp_filter_input2(index_final3,2),'r.');
scatter(temp_filter_input2(index_final4,1),temp_filter_input2(index_final4,2),'g.');
hold off;
grid on
ylabel('Elevation (km)')
xlabel('Along-track distance (km)')
%% Comparing with ATL08
load Data3_ATL08.mat;
figure();
scatter(temp_filter_input2(index_final,1),temp_filter_input2(index_final,2),'.');
hold on
index_atl08 = TBLHC_weak_alt08(:,5)>0;
scatter((TBLHC_weak_alt08(index_atl08,1)-TBLHC_weak_alt08(1,1)),TBLHC_weak_alt08(index_atl08,2),'.');
legend('Ours','ATL08');
